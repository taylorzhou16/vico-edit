...
vico-edit 支持三种视频生成后端，**按场景需求选择**（无绝对优先级）：

| 后端 | 模型 | 核心优势 | 局限 | 推荐使用场景 |
|------|------|---------|------|------------|
| **Kling Omni** | kling-v3-omni | image_list 多参考图、角色一致性最佳 | 场景控制较弱，首帧不确定 | 有人物的剧情视频、需保持人物一致性的系列镜头 |
| **Kling** | kling-v3 | 首帧精确控制、画面质感好、场景一致性强 | 需要额外的 Gemini 生首帧步骤 | 需要精确构图/场景控制、无人或人物不需严格一致的场景 |
| **Vidu** | viduq3-pro | 稳定、快速 | 无 multi_shot、无首尾帧 | 简单场景、快速原型、兜底 |

**核心权衡：人物一致性 vs 场景精确度**
- Omni 走参考图路径简单直接，但场景控制弱
- Kling-v3 走首帧路径画面质感和场景控制更好，但需要 Gemini 生成首帧

**自动选择逻辑**（未指定 `--backend` 时）：
- 提供 `--image-list` → 自动使用 kling-omni（唯一支持）
- 提供 `--tail-image` → 自动使用 kling（唯一支持）
- 无任何特殊参数 → 使用 vidu（最快）
...
```json
{
  ...
  "duration": 5,
  "shot_type": "dialogue",
  "description": "小美在赛车模拟器前",
  "generation_mode": "omni-video",
  "generation_backend": "kling-omni",
  "video_prompt": "小美（<<<image_1>>>）戴着耳机，在赛车模拟器前全神贯注。屏幕显示 F1 游戏画面。镜头缓慢推进。竖屏9:16构图。",
...
#### Kling Omni 多参考图 + multi_shot 示例

```json
{
  "shot_id": "scene3_multi",
  "duration": 7,
  "shot_type": "dialogue",
  "description": "小美和小明在维修区对话",
  "generation_mode": "omni-video",
  "generation_backend": "kling-omni",
  "video_prompt": "维修区场景，两人物交流",
  "reference_images": ["/path/to/xiaomei.jpg", "/path/to/xiaoming.jpg"],
  "multi_shot": true,
  "multi_shot_config": {
    "mode": "customize",
    "shots": [
      {"index": 1, "prompt": "中景：小美 <<<image_1>>> 站在维修区，看向远方", "duration": "3"},
      {"index": 2, "prompt": "中景：小明 <<<image_2>>> 走过来，说：听说你很快？", "duration": "4"}
    ]
  },
  "audio": true,
  "transition": "cut"
...
| `generation_mode` | 保留 | text2video / img2video / omni-video |
...
**仅当已注册人物参考图时考虑。有两条路径可选：**

#### 两种路径对比

| | Kling 路径（首帧控制） | Kling Omni 路径（参考图） |
|---|---|---|
| **流程** | 参考图 → Gemini 生分镜图 → Kling img2video | 参考图 → Kling Omni `--image-list` |
| **优势** | 画面质感好，场景精确可控 | 路径简单，无需 Gemini 中间步骤 |
| **一致性** | 多镜头间一致性难保证 | 同一参考图多镜头自动一致 |
| **适用** | 需要精确构图/场景控制 | 人物一致性优先的剧情视频 |

**选择建议**：
- 人物一致性优先 → **Kling Omni**（推荐，路径最短）
- 场景精确度优先 → **Kling + Gemini 首帧**

---

#### 路径 A：Kling Omni（推荐 - 角色一致性）

```
人物参考图 → Kling Omni（--image-list 参考图，prompt 用 <<<image_1>>> 引用）→ 视频
```
...
**如何选择 generation_backend？核心决策：人物一致性 vs 场景精确度**
...
```json
{
  ...
  "reference_images": ["/path/to/xiaomei_ref.jpg"],
  "notes": "Omni 自动保持人物一致性，无需 Gemini 生首帧"
}
```
...
### Kling / Kling Omni 多镜头模式（可选）

**Kling（`--backend kling`）和 Kling Omni（`--backend kling-omni`）均支持多镜头一镜到底**，可以在一次 API 调用中生成包含多个镜头的视频。Kling Omni 还可结合 `--image-list` 在多镜头中保持角色一致性。
...
# 视频生成（Kling 后端 - 文生视频/图生视频）
python ~/.claude/skills/vico-edit/vico_tools.py video --prompt <描述> --backend kling --duration 5 --output <输出>
python ~/.claude/skills/vico-edit/vico_tools.py video --image <首帧图> --prompt <描述> --backend kling --output <输出>
...
# Kling Omni 多参考图 + 多镜头
python ~/.claude/skills/vico-edit/vico_tools.py video --backend kling-omni --prompt "故事" --image-list <参考图1> <参考图2> --multi-shot --shot-type customize --multi-prompt '[{"index":1,"prompt":"<<<image_1>>> 镜头1","duration":"3"},{"index":2,"prompt":"<<<image_2>>> 镜头2","duration":"4"}]' --duration 7
...
#### Kling / Kling Omni 命令行参数说明

| 参数 | 适用后端 | 说明 |
|------|---------|------|
| `--backend kling` | kling | 使用 Kling v3 后端（首帧/首尾帧控制） |
| `--backend kling-omni` | kling-omni | 使用 Kling Omni 后端（参考图模式） |
...
| `--shot-type` | kling, kling-omni | 多镜头类型：`intelligence`（AI自动）或 `customize`（自定义） |
| `--multi-prompt` | kling, kling-omni | 自定义分镜的镜头列表（JSON格式） |
| `--audio` | kling, kling-omni | 启用音画同出 |
| `--mode` | kling, kling-omni | 生成模式：`std`（标准）或 `pro`（高质量） |
...
| **端点** | `/v2/text2video` `/v2/img2video` | `/v1/videos/text2video` `/v1/videos/image2video` | `/v1/videos/omni-video` |
...
**重要区别**：
- Kling 的 `--image` 是**首帧图**（视频从此图开始）
- Kling Omni 的 `--image-list` 是**参考图**（人物保持一致，但首帧不确定）
